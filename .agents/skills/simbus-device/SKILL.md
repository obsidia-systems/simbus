...
`modbus-tcp` for 502 clear vs 802 TLS. `opcua` is served (IANA 4840, None +
Anonymous, same YAML bank as Modbus). Do **not** add it to `devices/builtin/`
(Compose does not publish 4840). Dual-bind with `modbus-tcp` when the lab
needs both SCADA planes. Do not add `modbus-rtu`, `snmp-v2c`,
`mqtt-sparkplug`, or `bacnet-ip` unless the user asked for syntax-only;
`simbus check` accepts them, **boot refuses**.
