...
| [IANA `mbap` 502](https://www.iana.org/assignments/service-names-port-numbers/service-names-port-numbers.xhtml?search=mbap) | Well-known port. Not an IETF protocol RFC |
...
1. Listen TCP on the resolved port (`modbus.default_port` / `--port`).
2. Serve only `protocol: modbus-tcp`.
3. Share the engine register bank with the tick loop and the control plane.
...
id `0x0000` are tokio-modbus’s job (V1.0b §3.1).
...
4. Do not add RTU/TLS until [spec.md](spec.md) §4 flips `is_implemented`.
