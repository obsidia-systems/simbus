...
@app.command()
def start(
    device_type: str | None = typer.Option(  # noqa: UP007
        None,
        "--type",
        help="Built-in device type (e.g. generic-tnh-sensor)",
    ),
    file: str | None = typer.Option(  # noqa: UP007
        None,
        "--file",
        "-f",
        help="Path to a custom device YAML file",
    ),
    port: int | None = typer.Option(None, "--port", "-p", help="Modbus TCP port"),  # noqa: UP007
    name: str | None = typer.Option(None, "--name", "-n", help="Override device name"),  # noqa: UP007
    ...
    seed: int | None = typer.Option(None, "--seed", help="RNG seed for reproducibility"),  # noqa: UP007
) -> None:
    ...
    typer.echo(f"Starting '{name or device_type or file}' — Modbus :{modbus_port_display}  API :{api_port}")
    uvicorn.run(
        fastapi_app,
        host=host,
        port=api_port,
